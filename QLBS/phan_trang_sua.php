                    ...
                    <table class="table table-bordered">
                        ...
                        <tbody>
                            <?php 
								...
								// Thiết lập dữ liệu hiện thị tiếng việt
								mysqli_set_charset($con, "UTF8");
								
								//Chọn csdl cần lấy
								$db = mysqli_select_db($con, "ql_ban_sua");
								// Kiểm tra csdl đã chọn có tồn tại hay không
								if(!$db){
									echo "Không có csdl đã chọn!";
									exit;
								}
                                ...
                                $count  = mysqli_num_rows(mysqli_query($con, "SELECT * FROM sua"));
                               ...
                               // echo $page;
								...
								$sql = 'SELECT 
                                            sua.Ten_sua, 
                                            hang_sua.Teng_hang_sua,
                                            loai_sua.Ten_loai,
                                            sua.Trong_luong,
                                            sua.Don_gia
                                             FROM sua, hang_sua, loai_sua
                                where sua.Ma_hang_sua = hang_sua.Ma_hang_sua AND
                                 sua.Ma_loai_sua = loai_sua.Ma_loai_sua
                                 LIMIT '. $start . ",". $limit ;

								$result = mysqli_query($con, $sql);
								
								// Lặp lấy từng dữ liệu trong bảng
                                $stt =1;
